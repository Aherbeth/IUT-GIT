# Liens intéressants pour GIT / GITHUB

## GIT

- [cheatSheet](https://dev.to/sarath_pm/git-cheatsheet-414e)
- [Other list of useful commands](https://dev.to/g_abud/advanced-git-reference-1o9j)
- [Session interactive GIT](https://learngitbranching.js.org/?locale=fr_FR)
- [jeu git](https://ohmygit.org/)

## Documentation

### GIT

- [GIT official documentation](https://git-scm.com/docs)
- [Git Flow](https://nvie.com/posts/a-successful-git-branching-model/)

### Flows

- [Diagram Git Flow](https://nvie.com/files/Git-branching-model.pdf)
- [GitHub Flow](https://githubflow.github.io/)
- [conventionnal commits](https://www.conventionalcommits.org/en/v1.0.0/)

### Hooks

- [Git Hooks - Off](https://git-scm.com/book/en/v2/Customizing-Git-Git-Hooks)
- [Git Hooks - CheatSheet](https://githooks.com/)
- [Git Hooks](https://delicious-insights.com/fr/articles/git-hooks/)
- [Husky](https://github.com/typicode/husky)
- [Husky documentation](https://typicode.github.io/husky/#/)

### GitHub Actions

- [GHA documentation](https://docs.github.com/en/actions)

## Tools

- [GitHub CLI documentation](https://cli.github.com/manual/)
- [GitHub action events](https://docs.github.com/en/actions/using-workflows/events-that-trigger-workflows)

### GUI

- [Lazygit - git in terminal](https://github.com/jesseduffield/lazygit)
- [GitHub desktop app](https://desktop.github.com/)
- [GIT kraken](https://www.gitkraken.com/)
